...
class TestSafeStr:
    def test_string_pasa_igual(self):
        assert server._safe_str("hola") == "hola"

    def test_none_es_cadena_vacia(self):
        assert server._safe_str(None) == ""

    def test_nan_es_cadena_vacia(self):
        assert server._safe_str(float("nan")) == ""

    def test_numero_se_convierte_a_string(self):
        assert server._safe_str(42) == "42"
...
class TestFirstNonempty:
    def test_toma_la_primera_columna_no_vacia(self):
        ...
        assert server._first_nonempty(row, ["title", "abstract", "summary"]) == (
            "Real content"
        )

    def test_sin_columnas_preferidas_busca_por_nombre(self):
        row = pd.Series({"chunk_text_extra": "hola mundo", "other": "x"})
        out = server._first_nonempty(row, ["title", "abstract"])
        assert "hola mundo" in out

    def test_sin_nada_util_devuelve_vacio(self):
        row = pd.Series({"id": 1})
        assert server._first_nonempty(row, ["title"]) == ""


class TestGetCrossEncoder:
    def test_carga_una_sola_vez_y_cachea(self):
        fake_ce = MagicMock()
        with patch("server.CrossEncoder", return_value=fake_ce) as ce_cls:
            ce1 = server.get_cross_encoder("modelo-x")
            ce2 = server.get_cross_encoder("modelo-x")
...
class TestRerankWithCrossEncoder:
    ...
    def test_dataframe_vacio_lanza_value_error(self):
        with pytest.raises(ValueError):
            server.rerank_with_cross_encoder("q", pd.DataFrame())

    def test_fusiona_ce_y_score_denso(self):
        ce = MagicMock()
        ce.predict.return_value = [0.1, 0.9, 0.5]
        with patch("server.get_cross_encoder", return_value=ce):
            out = server.rerank_with_cross_encoder("q", self._df())
    ...
    def test_sin_fusion_usa_solo_ce(self):
        ...
        ce.predict.return_value = [0.1, 0.9, 0.5]
        with patch("server.get_cross_encoder", return_value=ce):
            out = server.rerank_with_cross_encoder(
    ...
    def test_respeta_batch_size(self):
        ...
        with patch("server.get_cross_encoder", return_value=ce):
            server.rerank_with_cross_encoder("q", self._df(), batch_size=1)
