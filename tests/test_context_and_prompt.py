...
class TestShorten:
    def test_texto_corto_no_se_trunca(self):
        assert server._shorten("hola", 100) == "hola"

    def test_texto_largo_se_trunca_con_elipsis(self):
        out = server._shorten("a" * 20, 10)
        assert out == "a" * 7 + "..."
        assert len(out) == 10

    def test_colapsa_espacios(self):
        assert server._shorten("hola   \n  mundo", 100) == "hola mundo"


class TestSplitAuthors:
    def test_vacio_devuelve_lista_vacia(self):
        assert server._split_authors("") == []

    def test_separa_por_punto_y_coma(self):
        assert server._split_authors("Perez, J.; Gomez, A.") == [
            "Perez, J.",
            "Gomez, A.",
        ]

    def test_separa_por_and_literal(self):
        assert server._split_authors("Perez and Gomez") == ["Perez", "Gomez"]

    def test_fallback_a_coma_si_no_hay_punto_y_coma(self):
        assert server._split_authors("Perez, Gomez, Diaz") == [
            "Perez",
            "Gomez",
            "Diaz",
        ]
...
class TestLastName:
    def test_vacio_devuelve_vacio(self):
        assert server._last_name("") == ""

    def test_formato_apellido_coma_nombre(self):
        assert server._last_name("Perez, Juan") == "Perez"

    def test_formato_nombre_apellido_toma_ultimo_token(self):
        assert server._last_name("Juan Perez") == "Perez"


class TestFormatAuthorsForMention:
    def test_sin_autores_devuelve_none(self):
        assert server._format_authors_for_mention("") is None

    def test_un_solo_nombre_sin_separadores(self):
        assert server._format_authors_for_mention("Cristina") == "Cristina"
    ...
    def test_autores_presentes_pero_sin_apellido_util_devuelve_none(self):
        ...
        with patch("server._split_authors", return_value=["   "]):
            assert server._format_authors_for_mention("x") is None

    def test_dos_autores(self):
        out = server._format_authors_for_mention("Perez, Juan; Gomez, Ana")
        assert out == "Perez y Gomez"

    def test_tres_o_mas_autores_usa_et_al(self):
        out = server._format_authors_for_mention(
    ...
    def test_un_solo_autor_formato_apellido_coma_nombre(self):
        ...
        # real observado, no necesariamente el deseado (ver informe).
        assert server._format_authors_for_mention("Perez, Juan") == "Perez y Juan"


class TestExtractYear:
    def test_extrae_anio_de_columna_year(self):
        row = pd.Series({"year": "2021"})
        assert server._extract_year(row) == "2021"

    def test_extrae_anio_de_cover_date(self):
        row = pd.Series({"cover_date": "2019-05-01"})
        assert server._extract_year(row) == "2019"

    def test_sin_anio_devuelve_none(self):
        row = pd.Series({"title": "sin fecha"})
        assert server._extract_year(row) is None
...
class TestBuildContextBlocks:
    ...
    def test_vacio_lanza_value_error(self):
        with pytest.raises(ValueError):
            server.build_context_blocks(pd.DataFrame())

    def test_arma_bloques_con_top_k(self):
        blocks = server.build_context_blocks(self._df(), top_k=1)
        assert len(blocks) == 1
...
class TestRenderFuentesFromBlocks:
    def test_formatea_cada_bloque_numerado(self):
        ...
        out = server.render_fuentes_from_blocks(blocks)
...
class TestComposePrompt:
    ...
    def test_incluye_idioma_objetivo_y_pregunta(self):
        prompt = server.compose_prompt("mi pregunta", self._blocks(), target_iso="en")
        assert "'en'" in prompt
        assert "mi pregunta" in prompt

    def test_target_iso_invalido_cae_a_es(self):
        prompt = server.compose_prompt("q", self._blocks(), target_iso="xx-yy")
        assert "'es'" in prompt

    def test_trunca_a_max_chars(self):
        prompt = server.compose_prompt(
