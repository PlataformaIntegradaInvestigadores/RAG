...
class TestGenerateWithOllamaHttp:
    def test_sin_target_iso_lanza_runtime_error(self):
        with pytest.raises(RuntimeError):
            server.generate_with_ollama_http("prompt", target_iso=None)

    def test_target_iso_invalido_lanza_runtime_error(self):
        with pytest.raises(RuntimeError):
            server.generate_with_ollama_http("prompt", target_iso="esp")
    ...
    def test_llamada_exitosa_devuelve_contenido(self):
        ...
        with patch("server.requests.post", return_value=fake_response) as post:
            out = server.generate_with_ollama_http(
        ...
        assert kwargs["json"]["model"] == server.OLLAMA_MODEL

    def test_respuesta_vacia_lanza_runtime_error(self):
        fake_response = MagicMock()
        fake_response.json.return_value = {"message": {"content": "   "}}
        with patch("server.requests.post", return_value=fake_response):
            with pytest.raises(RuntimeError):
                server.generate_with_ollama_http("prompt", target_iso="es")
