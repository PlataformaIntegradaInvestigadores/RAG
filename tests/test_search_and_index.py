...
class TestLoadPklAndModel:
    def test_carga_desde_pkl_y_cachea(self):
        ...
        with patch("builtins.open", mock_open(read_data=pkl_bytes)), patch(
            "server.SentenceTransformer", return_value=fake_model
        ) as st:
            model, meta_min = server.load_pkl_and_model(emb_max_seq_len=128)
            model2, meta_min2 = server.load_pkl_and_model()
    ...
    def test_usa_modelo_por_defecto_si_pkl_no_lo_especifica(self):
        ...
        with patch("builtins.open", mock_open(read_data=pkl_bytes)), patch(
            "server.SentenceTransformer", return_value=MagicMock()
        ) as st:
            server.load_pkl_and_model()
    ...
    def test_limita_max_seq_length_a_512(self):
        ...
        with patch("builtins.open", mock_open(read_data=pkl_bytes)), patch(
            ...
            server.load_pkl_and_model(emb_max_seq_len=9999)
...
class TestLoadFaiss:
    def test_carga_y_cachea_el_indice(self):
        ...
        with patch("server.faiss.read_index", return_value=fake_index) as read_index:
            idx1 = server.load_faiss()
            idx2 = server.load_faiss()
        read_index.assert_called_once_with(server.FAISS_PATH)
...
class TestLoadScopusCsv:
    def test_carga_y_normaliza_scopus_id_a_string(self):
        ...
        with patch("server.pd.read_csv", return_value=df):
            out = server.load_scopus_csv()
    ...
    def test_sin_columna_scopus_id_lanza_value_error(self):
        ...
        with patch("server.pd.read_csv", return_value=df):
            with pytest.raises(ValueError):
                server.load_scopus_csv()

    def test_cachea_entre_llamadas(self):
        df = pd.DataFrame({"scopus_id": [1]})
        with patch("server.pd.read_csv", return_value=df) as read_csv:
            server.load_scopus_csv()
            server.load_scopus_csv()
...
class TestE5EncodeQuery:
    def test_antepone_prefijo_query_y_normaliza(self):
        ...
        out = server.e5_encode_query(model, "IA en salud")
...
class TestSearchFullScopus:
    ...
    def test_une_topk_con_columnas_del_csv(self):
        ...
        with patch(
            "server.load_pkl_and_model", return_value=(model, meta_min)
        ), patch("server.load_faiss", return_value=index), patch(
            "server.load_scopus_csv", return_value=scopus_df
        ):
            out = server.search_full_scopus("consulta", topk=3)
    ...
    def test_sin_scopus_id_en_meta_min_lanza_value_error(self):
        ...
        with patch(
            ...
            with pytest.raises(ValueError):
                server.search_full_scopus("consulta", topk=3)
